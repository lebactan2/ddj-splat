...
//   DDJ-200 : mixxxdj/mixxx "Pioneer DDJ-200.midi.xml" (every number below is
...
const PROFILES = {
  ...
  // ── Pioneer DDJ-200 ─────────────────────────────────────────────────────────
  // Source: mixxxdj/mixxx "Pioneer DDJ-200.midi.xml" (status/midino pairs taken
  // verbatim from its <control> list; that file is in turn derived from the
  // official Pioneer DDJ-200 MIDI Message List).
  //
  // The DDJ-200 is the small 2-channel Bluetooth/USB unit, so it exposes far
  // fewer controls than the 400/FLX4:
  //   - no Beat FX section, no trim knobs, no master/headphone knobs
  //   - no dedicated loop buttons (loops live on the pads)
  //   - the shift layer is generated BY THE CONTROLLER: shifted controls send
  //     their own distinct notes (e.g. shift+CUE = 0x48, shifted pads move to
  //     ch8/ch10), so this profile never has to track shift state itself.
  //
  // Key number differences vs the DDJ-400 map above:
  //   - EQ is the MSB CCs 0x07 / 0x0B / 0x0F (the 400 profile reads the LSBs)
  //   - Channel fader is CC 0x13 MSB (0x33 is its LSB — ignored here)
  //   - Jog: 0x22 = scratch (top plate), 0x21 = bend ring, 0x29 = shift+jog seek
  //   - Filter/CFX ("Super knob") is ch6 CC 0x17 / 0x18 — same as the 400
  //
  // Controls the hardware simply doesn't have (chunk count, DOF, lens flare,
  // master zoom, FX depth) are unmapped — reach them on screen, or bind spare
  // buttons to them with GUIDED MAP / MIDI Learn (the override layer applies to
  // this built-in like any other).
  'ddj-200': {
    handleNoteOn(channel, note, velocity) {
      // ── Deck transport (ch0 = Deck A, ch1 = Deck B) ──
      if (channel === 0 || channel === 1) {
        const deckStr = channel === 0 ? 'a' : 'b';

        if (note === 0x0b) clickButton(`btn-play-${deckStr}`, velocity);  // Play/Pause
        if (note === 0x0c) clickButton(`btn-stop-${deckStr}`, velocity);  // Cue → Stop
        if (note === 0x58) clickButton(`sync-${deckStr}`, velocity);      // Sync

        // No loop buttons on this unit — the cue-set / shift+cue pair drives the
        // chunk loop range instead (0x47 = CUE SET, 0x48 = shift+CUE).
        if (note === 0x47) clickButton(`loop-in-${deckStr}`, velocity);
        if (note === 0x48) clickButton(`loop-out-${deckStr}`, velocity);

        // Headphone CUE (PFL) → arm the loop; shift+PFL → commit/exit it.
        if (note === 0x54) clickButton(`loop-active-${deckStr}`, velocity);
        if (note === 0x68) clickButton(`loop-toggle-${deckStr}`, velocity);

        // BPM TAP → cycle tempo range. (SHIFT itself, note 0x3F, is deliberately
        // left unmapped: the controller sends it on every shifted combo, so
        // acting on it would fire on each shift+X press too.)
        if (note === 0x60 && velocity > 0) { if (window._cycleTempoRange) window._cycleTempoRange(deckStr); }

        // Jog touch (0x36) — the DDJ-200 platter isn't touch-capacitive in a way
        // we need; rotation alone drives the jog, so this is intentionally a no-op.
      }

      // ── Performance pads: ch7 = Deck A, ch9 = Deck B, notes 0x00-0x07 ──
      if (channel === 7 && note >= 0 && note <= 7) triggerPad('a', note, velocity);
      if (channel === 9 && note >= 0 && note <= 7) triggerPad('b', note, velocity);

      // ── Shifted pads: ch8 = Deck A, ch10 = Deck B → camera viewpoint presets ──
      if (channel === 8  && note >= 0 && note <= 7) triggerCam('a', note, velocity);
      if (channel === 10 && note >= 0 && note <= 7) triggerCam('b', note, velocity);

      // ── Master / browse section (ch6) ──
      if (channel === 6) {
        if (note === 0x63) clickButton('btn-strobe', velocity);       // Headphones MASTER → strobe
        if (note === 0x78) clickButton('btn-reset-orient', velocity); // shift+ that → RESET VIEW
        if (note === 0x59) clickButton('btn-fx-toggle-a', velocity);  // browse/AutoDJ pair → FX on/off
        if (note === 0x5a) clickButton('btn-fx-toggle-b', velocity);
      }
    },
    ...
    handleCC(channel, cc, value) {
      ...
      if (channel === 0 || channel === 1) {
        ...
        if (cc === 0x22) spinJog(`jog-${deckStr}`, value);        // top plate = scratch
        if (cc === 0x29) spinJog(`jog-${deckStr}`, value, 0.12);  // shift+jog = coarse seek spin
        if (cc === 0x21) {                                        // bend ring = XZ translation nudge
          const ndelta = (value < 64 ? value : -(128 - value));
          const jogEl = document.getElementById(`jog-${deckStr}`);
          if (jogEl) jogEl.dispatchEvent(new CustomEvent('jognudge', { detail: { delta: ndelta * 0.003 } }));
        }

        // Rate/tempo MSB (LSB 0x20 ignored). The DDJ-200 has no physical pitch
        // fader — this only arrives from the software/Smart-Fader side, so it is
        // NOT inverted the way the DDJ-400's real fader is.
        if (cc === 0x00) mapSlider(`tempo-${deckStr}`, value);

        if (cc === 0x13) mapSlider(`vol-${deckStr}`, value);      // Channel fader MSB (LSB 0x33 ignored)
        if (cc === 0x07) mapSlider(`eq-hi-${deckStr}`, value);    // EQ High
        if (cc === 0x0b) mapSlider(`eq-mid-${deckStr}`, value);   // EQ Mid
        if (cc === 0x0f) mapSlider(`eq-low-${deckStr}`, value);   // EQ Low
      }

      // ── Mixer section (ch6) ──
      if (channel === 6) {
        if (cc === 0x1f) mapSlider('crossfader', value);          // Crossfader MSB (LSB 0x3F ignored)
        if (cc === 0x17) mapSlider('filter-a', value);            // CFX / Super knob, deck A
        if (cc === 0x18) mapSlider('filter-b', value);            // CFX / Super knob, deck B
      }
    },
  },
